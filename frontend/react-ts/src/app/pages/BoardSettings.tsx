...
/** How many rows each settings tab renders before its scroll sentinel appends more. */
...
export function BoardSettings() {
  ...
  // Each settings tab keeps its own viewport slice, so only the rows that fit
  // on screen are rendered; the rest come in as the user scrolls the page.
  const {
    visible: visibleConcepts,
    hasMore: hasMoreConcepts,
    sentinelRef: conceptsSentinelRef,
  } = useIncrementalList(boardConcepts, DISPLAY_PAGE_SIZE, id ? `${id}:concepts` : undefined);
  const {
    visible: visibleTags,
    hasMore: hasMoreTags,
    sentinelRef: tagsSentinelRef,
  } = useIncrementalList(allTags, DISPLAY_PAGE_SIZE, id ? `${id}:tags` : undefined);
  const {
    visible: visibleLogs,
    hasMore: hasMoreLogs,
    sentinelRef: logsSentinelRef,
  } = useIncrementalList(boardLogs, DISPLAY_PAGE_SIZE, id ? `${id}:logs` : undefined);
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-3">
            ...
            {isLoading ? (
              ...
              <>
                ...
                {hasMoreConcepts && <div ref={conceptsSentinelRef} className="h-px" aria-hidden="true" />}
              </>
            )}
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-3">
            ...
            {isLoading ? (
              ...
              <>
                ...
                {hasMoreTags && <div ref={tagsSentinelRef} className="h-px" aria-hidden="true" />}
              </>
            )}
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-3">
            ...
            {isLoading ? (
              ...
              <>
                ...
                {hasMoreLogs && <div ref={logsSentinelRef} className="h-px" aria-hidden="true" />}
              </>
            )}
          </motion.div>
        )}
